...
from bert import optimization_finetuning as optimization
...
flags.DEFINE_string(
    "output_dir", None,
    "The output_predict directory where the model checkpoints will be written.")
...
flags.DEFINE_integer("max_steps_without_decrease", 2000,
                     "How many steps make early stop.")#########20-9-15mart
...
class PaddingInputExample(object):
  ...
  the entire output_predict data won't be generated.
...
class SimProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir):
        file_path = os.path.join(data_dir, 'eval_data.txt')
        with open(file_path, 'r', encoding='utf-8') as f:
            dev_data = []
            index = 0
            for line in f.readlines():
    ...
    def get_test_examples(self, data_dir):
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for line in f.readlines():
                ...
                line = line.strip().split("<SEP>")
                text_a = tokenization.convert_to_unicode(str(line[1]))
                test_data.append(InputExample(guid=guid, text_a=text_a, text_b=None, label="3"))#随便给个目前存在的默认值
                index += 1
        return test_data
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  # segment.
  #
  # If you want to use the token-level output_predict, use model.get_sequence_output()
  # instead.
  ...
  with tf.variable_scope("loss"):
    ...
    probabilities = tf.argmax(logits, axis=-1, output_type=tf.int32)  ###################20-9-15
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      hook_dict={}#####20-11-28mart
      hook_dict['loss']=total_loss#20-11-28mart
      hook_dict['global_steps']=num_train_steps-tf.train.get_or_create_global_step()#20-11-28mart
      logging_hook = tf.train.LoggingTensorHook(hook_dict, every_n_iter=1)  #####20-9-14mart
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
        ...
        # auc = tf.metrics.auc(labels=label_ids, predictions=predictions, weights=is_real_example)#仅仅适用于二分类
        ##################20-9-26 add by mart
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    output_eval_file = os.path.join(FLAGS.output_dir, "eval_results16-layer24-4million-2.txt") # finetuning-layer24-4million###########20-9-22 mart
  ...
  if FLAGS.do_predict:
    ...
    output_predict_file = os.path.join(FLAGS.output_dir, "test_results.txt")################################20-9-15
